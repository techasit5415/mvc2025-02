<script>
    ...
    function logout() {
        document.cookie =
            "candidate_session=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT;";
        goto("/login");
    }
...
function canCancelApplication(deadline) {
  const today = new Date();
  const deadlineDate = new Date(deadline);
  return today < deadlineDate; // ยกเลิกได้ถ้ายังไม่ปิดรับสมัคร
}
    ...
    async function cancelApplication(jobId) {
        if (confirm("คุณต้องการยกเลิกการสมัครงานตำแหน่งนี้หรือไม่?")) {
            try {
                const formData = new FormData();
                formData.append("job_id", jobId);
...
<div class="candidate-container">
    <header class="candidate-header">
        <div>
            ...
            {#if data.candidate}
                <p>
                    สวัสดี {data.candidate.first_name}
    ...
    <main class="candidate-content">
        ...
        <div class="jobs-grid">
            {#each sortedJobs as job}
                <div
                    class="job-card"
                    class:applied={data.appliedJobIds.includes(job.job_id)}
                >
                    ...
                    <div class="job-body">
                        ...
                        <div class="job-meta">
                            <div
                                class="deadline"
                                class:soon={isDeadlineSoon(job.deadline)}
                            >
                                <span class="label">ปิดรับสมัคร:</span>
                                <span class="date"
                                    >{formatDateTime(job.deadline)}</span
                                >
                            </div>
                    ...
                    <div class="job-footer">
                        {#if data.appliedJobIds.includes(job.job_id)}
                            <div class="applied-actions">
                                <span class="applied-status">✓ สมัครแล้ว</span>
                                <button
                                    class="cancel-btn"
                                    on:click={() =>
                                        cancelApplication(job.job_id)}
                                >
                                    ยกเลิกสมัคร
                                </button>
                            </div>
